...
Batch_Preparat::Batch_Preparat( PlayerChara *PcC, Stage *StgC, Enemy *EneC, Camera *Cam){
	...
	*/
	...
	/*メインウェポンからデータを読み込みます*/
	PCWp_hsids[0][0][0] = PcC->wp_data[0][0][0];//武器モデル
	PCWp_hsids[0][0][1] = PcC->wp_data[0][0][2];//武器の当たり判定モデル
	PCWp_hsids[0][0][2] = PcC->wp_data[0][0][3];//使用済み弾薬モデル
	/*サブウェポンからデータを読み込みます*/
	PCWp_hsids[0][1][0] = PcC->wp_data[1][0][0];
	PCWp_hsids[0][1][1] = PcC->wp_data[1][0][2];
	PCWp_hsids[0][1][2] = PcC->wp_data[1][0][3];
	/*グレネード系からデータを読み込みます*/
	PCWp_hsids[0][2][0] = PcC->wp_data[2][0][0];
	PCWp_hsids[0][2][1] = PcC->wp_data[2][0][2];
	PCWp_hsids[0][2][2] = PcC->wp_data[2][0][3];
...
int Batch_Preparat::BatchSpriteSet( PlayerChara *PcC){
	...
	if( PcC->Wp_equipment == -1){//武器なしなら
				...
				//銃の弾薬についての倍率変換
				SpriteData[5][0] = ( PcC->wp_data[PcC->Wp_equipment][3][0] / PcC->wp_data[PcC->Wp_equipment][1][2] ) * 100.0f;

				//銃のマガジンについての倍率変換
				SpriteData[6][0] = ( PcC->wp_data[PcC->Wp_equipment][3][1]/ PcC->wp_data[PcC->Wp_equipment][1][3] ) * 100.0f;

				//表示すべき画像をSpriteIDsに代入する
				SpriteIDs[9] = PcC->wp_data[PcC->Wp_equipment][0][1];
	}
