...
private:
	...
	map<string, NPC_t> NPC;

	/* 変数の宣言 */
	int DeadEnemys;// 殺された状態になっている、敵の数
	bool DeadEnemyFlag;
...
//	int EnemyNum;//現在ロードされているモデルの数
	...
	int StatusReset();// NPCクラスの状態を初期に戻します(リスタート等で利用)
	int EventPublish();// イベント関連の統括を行う

	/* アシスト関数 */
	map<string, NPC_t>::iterator NPC_endit();
	map<string, NPC_t>::iterator Get_NPC_begin();
